			...
			return $q(function(resolve, reject) {
				...
				if(foto._id) {
					recursoFoto.update({fotoId: foto._id}, foto, function() {
						resolve({
							mensagem: 'Formulário de ' + foto.titulo + ' atualizado com sucesso',
							inclusao: false
						});
					}, function(erro) {
						console.log(erro);
						reject({
							mensagem: 'Não foi possível atualizar o formulário ' + foto.titulo
						});
					});
					...
					recursoFoto.save(foto, function() {
						resolve({
							mensagem: 'Formulário de' + foto.titulo + ' incluído com sucesso',
							inclusao: true
						});
					}, function(erro) {
						alert('Não Resolveu');
						reject({
							mensagem: 'Não foi possível incluir as informações no formulário' + foto.titulo
						});
					});
				}
			});
